...
- `AppConfiguration` is the canonical portable, JSON-bindable schema. Load it with `Microsoft.Extensions.Configuration`, compile its HID key tokens through `ConfigurationCompiler`, and use `JsonAppConfigurationStore` only for atomic app-owned writes. Hosts choose filesystem locations; legacy Windows VK migration belongs in a Windows compatibility layer.
- `IInputFilterRuntime` is the platform-host contract used by UI code to start, stop, configure, and observe native capture. Platform adapters own capture and call the synchronous Core filters; they must safely dispose replaced filters so pending `BlockRelease` timers cannot survive reconfiguration.
- `AppProfileService` and `GameProfileCoordinator` hold portable profile-selection and game-switching policy. Native profile paths and game process monitoring remain platform implementations.
...
- Game-list refresh logic is shared and portable (`DiscordGameListUpdater`). Each platform-specific app will invoke this logic as is appropriate for its OS.
- Structured filter diagnostics are versioned JSON Lines events (`FilterDiagnosticEvent`) emitted through `IFilterDiagnosticSink`. The `Core.Heatmap` namespace consumes and streams/aggregates those events, renders self-contained generic HID HTML, and retains a legacy Windows text-log importer only for historical logs.
- `G915Fix.Desktop` serves as the shared UI abstraction layer for the platform-specific projects.